...
#define sensor_O3  25
//const double wind_speed_Pin = 39;  // Analog input pin that the potentiometer is attached to
const int wind_speed_Pin = 15;  // Analog input pin that the potentiometer is attached to
const int wind_dir_Pin = 36;  // Analog input pin that the potentiometer is attached to
...
bool read_all()
{
  ...
  doc["WindSpRAW"] = readWindSpeedRAWdata();
  doc["WindDir"] = readWindDir();
  doc["WindDirRAW"] = readWindDirRAWdata();
  ...
  Send_command("AT+CHTTPSEND=0,1,\"/aj_nitigan_3RAW.php\",,\"application/json\"," + Payload);
  Serial.println("Command Status : " + String(cmm_state));
...
void Send_command(String cmd)
{
  ...
  do
  {
    ...
    Sim_res = Wait_module_res(5000, "OK");
    fail_cunt ++;
...
String readWindSpeed(){
  wind_speed_adc = analogRead(wind_speed_Pin);
  wind_speed_voltage = wind_speed_adc * (5.0  / 4095.0);
  //wind_speed = mapf(wind_speed_voltage, 0.17, 3.3, 1, 20); 
  wind_speed = (wind_speed_voltage * 30.0)/5.0;
  if(wind_speed < 0)wind_speed = 0;
  
  return String(wind_speed);
}
String readWindSpeedRAWdata(){
  return String(wind_speed_adc);
}

String readWindDir(){
  wind_dir_adc = analogRead(wind_dir_Pin) * 2;
  if(wind_dir_adc >= 0 and wind_dir_adc < 580)wind_dir = "N";
  else if(wind_dir_adc >= 580 and wind_dir_adc < 1470)wind_dir = "NE";
  else if(wind_dir_adc >= 1470 and wind_dir_adc < 2330)wind_dir = "E";
  else if(wind_dir_adc >= 2330 and wind_dir_adc < 3240)wind_dir = "SE";
  else if(wind_dir_adc >= 3240 and wind_dir_adc < 4176)wind_dir = "S";
  else if(wind_dir_adc >= 4176 and wind_dir_adc < 4976)wind_dir = "SW";
  else if(wind_dir_adc >= 4976 and wind_dir_adc < 5870)wind_dir = "W";
  else if(wind_dir_adc >= 5870 and wind_dir_adc < 6040)wind_dir = "NW";
  else wind_dir = "UN";
...
String readWindDir(){
  wind_dir_adc = analogRead(wind_dir_Pin) * 2;
  if(wind_dir_adc >= 0 and wind_dir_adc < 1023)wind_dir = "N";
  else if(wind_dir_adc >= 1023 and wind_dir_adc < 2047)wind_dir = "NE";
  else if(wind_dir_adc >= 2047 and wind_dir_adc < 3070)wind_dir = "E";
  else if(wind_dir_adc >= 3070 and wind_dir_adc < 4093)wind_dir = "SE";
  else if(wind_dir_adc >= 4093 and wind_dir_adc < 5116)wind_dir = "S";
  else if(wind_dir_adc >= 5116 and wind_dir_adc < 6139)wind_dir = "SW";
  else if(wind_dir_adc >= 6139 and wind_dir_adc < 7162)wind_dir = "W";
  else if(wind_dir_adc >= 7162 and wind_dir_adc < 8190)wind_dir = "NW";
  else wind_dir = "UN";
  return String(wind_dir);
}

*/
String readWindDirRAWdata(){
  return String(wind_dir_adc);
}
